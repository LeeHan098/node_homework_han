...
router.post('/comments/:postid', async(req,res) => {
  ...
  try{
    ...
    return res.status(404).json({errorMessage:"postid를 찾을 수 없습니다."})
  }
  try{
    const createdComment = await Comments.create({text:text, createdAt:createdAt,postId:postid})
    res.status(201).json({comment: createdComment})
    console.log("메세지 저장")
  }
  catch{
    res.status(400).json({success: false, errorMessage: "생성에 실패했습니다."})
  }
})
...
router.put('/comments/:_id/', async (req,res) => {
  ...
  console.log({_id})
  if(!text){
    return res.status(404).json({errorMessage: "수정할 text가 비었습니다."})
  }
  try{
